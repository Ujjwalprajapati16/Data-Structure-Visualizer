...
class ShowDataList extends React.Component {
  constructor(props) {
    super(props);
    const { dataStates = [], changeStop = () => {} } = props;
    this.state = {
      showdata: false,
      showstring: false,
      string: null,
  ...
  changeShowString = (string = null) => {
    this.setState((prevState) => ({
      showstring: !prevState.showstring,
      string: string,
    }));
  };
  ...
  dataScript = (data) => {
    return (
      <div className="datascript" key={data.name}>
        <div className="dataname">Name: {data.name.substring(0, 20)}</div>
        <button className="consolelog" onClick={() => console.log(data.value)}>
          Console.log
        </button>
        <button
          className="toString"
          onClick={() => this.changeShowString(data.value.toString())}
        >
          toString
        </button>
        {data.value.classname === "List" ? (
          <button
            ...
            onClick={() => this.viewContainer(data)}
          >
            View
          </button>
        ) : null}
    ...
    return (
      <div className="showdata">
        <button
          ...
          onClick={() => this.changeShowdata(false)}
        >
          Show Data
        </button>
        {this.state.showdata ? (
          <div className="coverDom2">
            <div className="dataContent">
              ...
              <button className="closeshowdata" onClick={this.changeShowdata}>
                close
              </button>
            </div>
          </div>
        ...
        {this.state.showstring ? (
          <div className="dataContent">
            ...
            <button className="closeshowdata" onClick={this.changeShowString}>
              close
            </button>
          </div>
        ) : null}
