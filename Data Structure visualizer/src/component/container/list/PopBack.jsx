  ...
  if (count) {
    express.push(
      <text
        key={keyid}
        id="back-text"
        className="popBackMoving animate-ListPopBackMove"
        style={{
    ...
    express.push(
      <text
        key={keyid}
        className="pqPopSizeErase animate-ListPopBackCameOut"
    ...
    express.push(
      <text
        key={keyid}
        className="pqPopSizeEmerge animate-ListPopBackCameOut"
        style={{ animationDuration: `${duration * 2}s` }}
        x={interval}
        y={20}
        width={30}
    ...
    express.push(
      <text key={keyid} x={interval} y={60} width={30} height={15}>
        error: no element but pop
      </text>
    );
    keyid += 1;
    express.push(
  ...
  while (itr !== list.rend() && count > 0) {
    ...
    if (count === (list.size() > 5 ? 5 : list.size())) {
      DataNode({
        ...
        ani_delay: "0s",
        ani_dur: `${duration}s`,
        className: "removing animate-ListPopBackCameOut",
        data: data.toString(),
        x: interval * count + width * (count - 1),
        y: 50,
        width: width,
    ...
    if (i === 5) {
      ...
      if (count - 1 === i || i === 4) {
        Arrow(
          ...
          70,
          "removing animate-ListPopBackCameOut",
          keyid
        ).map((n) => express.push(n));
        keyid += 1;
        Arrow(
    ...
    <svg className="w-full h-full">
      ...
      <style>
        {`
          ...
          .removing {
            animation-name: ListPopBackCameOut;
            animation-timing-function: ease-in;
            animation-fill-mode: forwards;
          }

          .popBackMoving {
            animation-name: ListPopBackMove;
            animation-fill-mode: forwards;
          }
        `}
      </style>
